(function () {
  ...
  */
  ...
  window.addEventListener('load', function () {
    ...
    var sidebar = new Sidebar(sidebarContainer);
    var tabsContainer = sidebar.add('tabs', 'Tabs');
    sidebar.add('controls', 'Controls');
    var tabs = new Tabs(tabsContainer);
    ...
    spinner.min = 0;
    spinner.max = 10;
    spinner.value = 5; // spinner.on('change', e => alert(`spinner: ${e.detail}`));

    var sld = new Slider1(ctrl.querySelector('.slider-single'), {
      min: 0,
      max: 100
    });
    sld.hi = 14; // sld.on('stop', e => alert(`slider: ${sld.lo} - ${sld.hi}`));

    var rng = new Interval(ctrl.querySelector('.range-single'), {
      min: 1,
      max: 21,
      slider: Slider1
    });
    rng.hi = 14;
    var dbl = new Slider2(ctrl.querySelector('.slider-double'), {
      min: 1,
      max: 21
    });
    dbl.lo = 4;
    dbl.hi = 12;
    var dr = new Interval(ctrl.querySelector('.range-double'), {
      min: 1,
      max: 21,
      slider: Slider2
    });
    dr.lo = 4;
    dr.hi = 18;
    var tpl = new Slider3(ctrl.querySelector('.slider-triple'), {
      min: 1,
      max: 21
    });
    tpl.lo = 4;
    tpl.hi = 18;
    tpl.mi = 12; // tpl.on('change', console.log);

    var tr = new Interval3(ctrl.querySelector('.range-triple'), {
      min: 1,
      max: 21
    });
    tr.lo = 4;
    tr.hi = 18;
    tr.mi = 12; // tr.on('change', () => {
